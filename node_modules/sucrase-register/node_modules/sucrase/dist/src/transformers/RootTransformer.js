...
const FlowTransformer_1 = require("./FlowTransformer");
const ImportTransformer_1 = require("./ImportTransformer");
const JSXTransformer_1 = require("./JSXTransformer");
const NumericSeparatorTransformer_1 = require("./NumericSeparatorTransformer");
const OptionalCatchBindingTransformer_1 = require("./OptionalCatchBindingTransformer");
const ReactDisplayNameTransformer_1 = require("./ReactDisplayNameTransformer");
const TypeScriptTransformer_1 = require("./TypeScriptTransformer");
class RootTransformer {
    constructor(sucraseContext, transforms, filePath) {
        ...
        if (transforms.includes("jsx")) {
            this.transformers.push(new JSXTransformer_1.default(this, tokenProcessor, importProcessor, this.nameManager, filePath));
            this.transformers.push(new ReactDisplayNameTransformer_1.default(this, tokenProcessor, importProcessor));
        }
        if (transforms.includes("imports")) {
            const shouldAddModuleExports = transforms.includes("add-module-exports");
            this.transformers.push(new ImportTransformer_1.default(this, tokenProcessor, importProcessor, shouldAddModuleExports));
        }
        if (transforms.includes("flow")) {
            this.transformers.push(new FlowTransformer_1.default(this, tokenProcessor));
        }
        if (transforms.includes("typescript")) {
            if (!transforms.includes("imports")) {
                throw new Error("The TypeScript transform without the import transform is not yet supported.");
            }
            this.transformers.push(new TypeScriptTransformer_1.default(this, tokenProcessor));
        }
    }
    transform() {
        this.tokens.reset();
        this.processBalancedCode();
        const shouldAddUseStrict = this.transformers.some((t) => t instanceof ImportTransformer_1.default);
    ...
    processClass() {
        const classInfo = getClassInfo_1.default(this, this.tokens);
